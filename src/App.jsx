...
const googleAuthErrorMessage=(error)=>{
  const code=String(error?.code||"");
  if(code.includes("popup-closed-by-user")) return "Jendela Google ditutup sebelum login selesai.";
  if(code.includes("popup-blocked")) return "Browser memblokir jendela Google. Izinkan pop-up lalu coba lagi.";
  if(code.includes("unauthorized-domain")) return "Domain ini belum diizinkan untuk login Google. Hubungi admin.";
  if(code.includes("operation-not-allowed")) return "Login Google belum aktif di Firebase Authentication.";
  ...
  if(code.includes("internal-error")) return "Komponen login Google belum bisa dimuat. Muat ulang halaman lalu coba lagi.";
  return "Login Google belum berhasil. Muat ulang halaman atau gunakan email dan password.";
};
const INIT={
...
// ─── GOAL CARD ────────────────────────────────────────────────────────────────
const GoalCard=({g,dompetList,onDelete,onTambah,onGunakan,onSelesai,onSourceChange,onHistory,lang="id"})=>{
  const T=useT();
  const [inp,setInp]=useState("");const [showCalc,setShowCalc]=useState(false);
  ...
  return(
    <div className="feature-item-card goal-card" style={{background:T.card,borderRadius:14,padding:18,border:`1.5px solid ${pct>=100?T.okBorder:T.border}`,boxShadow:T.shadow,transition:"background .3s,border-color .3s",minWidth:0}}>
      ...
      {!showUse&&pct<100&&<div className="goal-fund-row" style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) auto",gap:8,alignItems:"center",width:"100%"}}>
      ...
      {!showUse&&<div className="goal-secondary-actions" style={{display:"grid",gridTemplateColumns:pct>=100?"minmax(0,1fr) minmax(0,1fr)":"minmax(0,1fr)",gap:8,marginTop:pct<100?8:0}}>
        {pct>=100&&<Btn onClick={()=>onSelesai(g.id)} ch={"✓ "+(lang==="en"?"Mark Done":"Selesai")} c="#16A34A" style={{width:"100%",padding:10}}/>}
        {km>0&&<Btn onClick={()=>setShowUse(true)} ch={lang==="en"?"Use funds":"Gunakan dana"} outline style={{width:"100%",padding:10}}/>}
      </div>}
      <Btn onClick={()=>onHistory?.(g.id)} ch={lang==="en"?"Balance history":"Riwayat saldo"} c={T.info} outline style={{width:"100%",padding:10,marginTop:8}}/>
      {showUse&&<div className="goal-use-panel" style={{display:"grid",gap:8,padding:11,borderRadius:11,background:T.cardAlt,border:`1px solid ${T.border}`}}>
        ...
        <div className="goal-use-actions" style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) auto",gap:8}}>
          <Btn onClick={()=>{if(onGunakan?.(g.id,useAmount,useDescription)!==false){setUseAmount("");setUseDescription("");setShowUse(false);}}} ch={lang==="en"?"Record usage":"Catat penggunaan"} style={{width:"100%",padding:10}}/>
          <Btn onClick={()=>{setShowUse(false);setUseAmount("");setUseDescription("");}} ch={lang==="en"?"Cancel":"Batal"} outline style={{padding:10}}/>
        </div>
      </div>}
    </div>
  );
...
export default function App(){
  ...
  const updateAdminApproval = async (uid, approvalStatus) => {
    setAdminLoading(true);
    try{
      const currentUser = adminUsers.find(user=>user.uid===uid) || {};
      const data = await authedJson("/api/admin/users", {
        ...
        body:JSON.stringify({
          ...
          buyerEmail: currentUser.buyerEmail || currentUser.email || "",
  ...
  // Data lama dapat memakai ID dompet selain 1. Pastikan nilai internal form
  // selalu sama dengan opsi yang benar-benar terlihat di dropdown.
  useEffect(()=>{
    if(!s.dompet.length)return;
    const firstId=s.dompet[0].id;
    const secondId=s.dompet[1]?.id??firstId;
    setTxForm(form=>({
      ...form,
      dompetId:hasWallet(s.dompet,form.dompetId)?form.dompetId:firstId,
      dompetTo:hasWallet(s.dompet,form.dompetTo)?form.dompetTo:secondId,
    }));
    setBulkRows(rows=>rows.map(row=>hasWallet(s.dompet,row.dompetId)?row:{...row,dompetId:firstId}));
...
4. Gunakan dana goal:
{"action":"gunakan_goal","nama":"nama goal","jumlah":300000,"ket":"keperluan penggunaan"}
  ...
  const handleAiSend = async (presetText="") => {
    ...
    try {
      ...
      if(extendedAiAction) {
        ...
        } else if(parsed.action==="gunakan_goal") {
          const goal=aiGoal(parsed.nama);
          const jumlah=aiAmount(parsed.jumlah, parsed.jml, parsed.nominal);
          const description=cleanAiText(parsed.ket||parsed.deskripsi||parsed.keperluan);
          if(!goal) aiDone(`⚠️ Goal "${parsed.nama}" tidak ditemukan. Goal yang ada: ${s.goals.map(g=>g.nama).join(", ")||"belum ada"}`);
          else if(!jumlah) aiDone("⚠️ Nominal dana Goal yang dipakai belum jelas.");
          else if(jumlah>N(goal.kumpul)) aiDone(`⚠️ Dana Goal ${goal.nama} tidak cukup. Tersedia Rp ${aiMoney(N(goal.kumpul))}.`);
          else if(!description) aiDone("⚠️ Jelaskan dana Goal dipakai untuk apa.");
          else {
            const spendTx={id:Date.now(),tipe:"pengeluaran",tgl:today(),ket:`Gunakan Goal: ${goal.nama} - ${description}`,jml:String(jumlah),dompetId:goal.dompetId||"",goalSpendId:goal.id,katId:autoKat(description),bulan:s.bulan,tahun:s.tahun};
            setS(p=>({...p,
              goals:p.goals.map(g=>sameId(g.id,goal.id)?{...g,kumpul:String(Math.max(N(g.kumpul)-jumlah,0)),history:[...(g.history||[]),{tgl:today(),jml:String(jumlah),dompetId:g.dompetId||"",tipe:"penggunaan",ket:description,txId:spendTx.id}]}:g),
              txs:[spendTx,...p.txs]
            }));
            aiDone(`🎯 **Dana Goal digunakan!**\n\n${goal.nama}\n- Rp ${aiMoney(jumlah)}\nUntuk: ${description}\n\nSaldo dompet tidak dipotong lagi karena sudah berkurang saat setoran Goal.`);
            showToast("Dana Goal digunakan via AI");
          }
...
Saldo amplop bertambah.`}]);
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      doc.autoTable({
        ...
        body:txRows,
        columnStyles:{
          0:{cellWidth:19,fontSize:7,textColor:C.gray},
          1:{cellWidth:8, halign:"center",fontSize:7,fontStyle:"bold"},
          2:{cellWidth:48,fontSize:7.5},
          3:{cellWidth:27,halign:"center",fontSize:7},
          4:{cellWidth:20,halign:"center",fontSize:7},
          5:{cellWidth:30,halign:"right",fontSize:7.5,fontStyle:"bold"},
          6:{cellWidth:30,halign:"right",fontSize:7.5,fontStyle:"bold"},
        },
  ...
  const addUt=()=>{
    ...
    setS(p=>{
      ...
      const sourceTx=isReceivable&&utForm.sourceType==="wallet"
        ...
        : null;
      return {...p,utang:[debt,...p.utang],goals:isReceivable&&utForm.sourceType==="goal"?p.goals.map(g=>sameId(g.id,utForm.sourceId)?{...g,kumpul:String(Math.max(N(g.kumpul)-amount,0)),history:[...(g.history||[]),{tgl:utForm.tgl,jml:String(-amount),tipe:"piutang_keluar",ket:`Dipinjamkan: ${utForm.nama}`}]}:g):p.goals,dompet:sourceTx?applyTransactionToWallets(p.dompet,sourceTx):p.dompet,txs:sourceTx?[sourceTx,...p.txs]:p.txs};
    });
  ...
  const catatCicilan=(uid,jml,destinationType,destinationId)=>{
    ...
    setS(p=>{
      ...
      return {...p,
        ...
        goals:receivesPayment&&destinationType==="goal"?p.goals.map(g=>sameId(g.id,destinationId)?{...g,kumpul:String(N(g.kumpul)+N(jml)),history:[...(g.history||[]),{tgl:today(),jml:String(N(jml)),tipe:"pengembalian_piutang",ket:`Pengembalian piutang: ${debt?.nama}`,txId:paymentTx.id}]}:g):p.goals,
  ...
  const gunakanGoalDana=(gid,jml,description)=>{
    const amount=N(jml);
    const goal=s.goals.find(item=>sameId(item.id,gid));
    if(!goal){showToast("Goal tidak ditemukan.");return false;}
    if(amount<=0){showToast("Nominal penggunaan harus lebih dari nol.");return false;}
    if(amount>N(goal.kumpul)){showToast(`Dana Goal tidak cukup. Tersedia ${IDRs(N(goal.kumpul))}.`);return false;}
    const cleanDescription=String(description||"").trim();
    if(!cleanDescription){showToast("Tulis dana Goal dipakai untuk apa.");return false;}
    setS(previous=>{
      const currentGoal=previous.goals.find(item=>sameId(item.id,gid));
      if(!currentGoal||amount>N(currentGoal.kumpul)) return previous;
      const spendTx={
        id:Date.now(),tipe:"pengeluaran",tgl:today(),
        ket:`Gunakan Goal: ${currentGoal.nama} - ${cleanDescription}`,
        jml:pN(amount),dompetId:currentGoal.dompetId||"",goalSpendId:currentGoal.id,
        katId:autoKat(cleanDescription||currentGoal.nama),bulan:previous.bulan,tahun:previous.tahun,
      };
      return {...previous,
        goals:previous.goals.map(item=>!sameId(item.id,gid)?item:{...item,
          kumpul:String(Math.max(N(item.kumpul)-amount,0)),
          history:[...(item.history||[]),{tgl:today(),jml:pN(amount),dompetId:item.dompetId||"",tipe:"penggunaan",ket:cleanDescription,txId:spendTx.id}],
        }),
        // Dana sudah dipotong dari dompet saat setoran Goal.
  ...
  const deleteTx=(tx)=>confirmDelete({
    ...
    onConfirm:()=>{
      ...
      }else if(tx.tipe==="pengeluaran"&&tx.goalSpendId){
        newGoals=p.goals.map(g=>{
          if(!sameId(g.id,tx.goalSpendId)) return g;
          const history=[...(g.history||[])];
          let historyIndex=history.findIndex(entry=>sameId(entry.txId,tx.id));
          if(historyIndex<0){
            for(let i=history.length-1;i>=0;i--){
              if(history[i].tipe==="penggunaan"&&N(history[i].jml)===jmlNum&&(!tx.tgl||history[i].tgl===tx.tgl)){historyIndex=i;break;}
            }
          }
          if(historyIndex>=0) history.splice(historyIndex,1);
          return {...g,kumpul:String(N(g.kumpul)+jmlNum),history};
        });
  ...
  if(!fireUser) return (
    <div style={{minHeight:"var(--app-height, 100dvh)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#2E1065 0%,#1A0A2E 100%)",padding:"max(24px, env(safe-area-inset-top, 0px)) max(24px, env(safe-area-inset-right, 0px)) max(24px, env(safe-area-inset-bottom, 0px)) max(24px, env(safe-area-inset-left, 0px))",fontFamily:"system-ui,sans-serif"}}>
      <div style={{width:"100%",maxWidth:360,display:"flex",flexDirection:"column",alignItems:"center",gap:0}}>
        ...
        <button type="button" onClick={handleGoogleSignIn} disabled={authBusy} aria-busy={authBusy} style={{width:"100%",padding:"14px 20px",borderRadius:14,border:"none",background:"white",color:"#1F1F1F",fontWeight:700,fontSize:15,cursor:authBusy?"wait":"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:12,boxShadow:"0 4px 20px rgba(0,0,0,0.3)",fontFamily:"inherit",transition:"all 0.2s",opacity:authBusy?0.7:1}}>
...
@media(max-width:767px){
  ...
  .goal-use-actions{grid-template-columns:1fr!important;}
  .goal-use-actions>.btn-go{width:100%!important;}
}
...
@container (max-width:340px){
  .goal-secondary-actions{grid-template-columns:1fr!important;}
      ...
      {modal&&(
        <div className={`modal-overlay ${modalClosing?"closing":""}`} style={{cursor:"pointer",position:"fixed",touchAction:"none",overscrollBehavior:"none",inset:0,background:"rgba(0,0,0,.55)",display:"flex",alignItems:isMobile?"flex-end":"center",justifyContent:"center",zIndex:1000,padding:isMobile?0:"16px",paddingTop:isMobile?"max(12px, env(safe-area-inset-top))":undefined}} onClick={()=>closeModal()}>
          <div className={`modal-pop ${modalClosing?"closing":""}`} style={{cursor:"pointer",background:T.card,borderRadius:isMobile?"24px 24px 0 0":20,padding:isMobile?"20px max(18px, env(safe-area-inset-right)) calc(env(safe-area-inset-bottom, 0px) + 24px) max(18px, env(safe-area-inset-left))":"26px",width:"100%",maxWidth:isMobile?"100%":520,maxHeight:isMobile?"min(88svh, calc(var(--app-height, 100dvh) - 12px))":"min(92vh, calc(var(--app-height, 100dvh) - 32px))",overflowY:"auto",overflowX:"hidden",color:T.text,WebkitOverflowScrolling:"touch"}} onClick={e=>e.stopPropagation()}>
            ...
            {modal.type==="goalHistory"&&(()=>{
              const goal=s.goals.find(item=>sameId(item.id,modal.goalId));
              const rows=buildGoalHistoryTimeline(goal,N);
              const historyLabel=entry=>{
                if(entry.tipe==="penggunaan") return lang==="en"?"Funds used":"Dana digunakan";
                if(entry.tipe==="piutang_keluar"||String(entry.ket||"").startsWith("Dipinjamkan")) return lang==="en"?"Lent from Goal":"Dipinjamkan dari Goal";
                if(entry.tipe==="pengembalian_piutang"||String(entry.ket||"").startsWith("Pengembalian")) return lang==="en"?"Receivable returned":"Pengembalian piutang";
                return lang==="en"?"Added to Goal":"Setoran ke Goal";
              };
              return <>
                <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
                  <span style={{width:44,height:44,borderRadius:14,background:T.infoBg,display:"grid",placeItems:"center",fontSize:22}}>{uiIcon(goal?.icon||"GOAL")}</span>
                  <div style={{minWidth:0}}><div style={{fontSize:17,fontWeight:900,color:T.text}}>{goal?.nama||"Riwayat Goal"}</div><div style={{fontSize:11,color:T.muted}}>Saldo sekarang {IDRs(N(goal?.kumpul))} · {rows.length} perubahan</div></div>
                </div>
                <div style={{maxHeight:isMobile?"58svh":440,overflowY:"auto",borderTop:`1px solid ${T.border}`}}>
                  {rows.length?rows.map((entry,index)=>{
                    const incoming=entry.delta>=0;
                    const wallet=findWallet(s.dompet,entry.dompetId);
                    return <div key={`${entry.txId||entry.tgl||"entry"}-${entry.index}-${index}`} style={{display:"grid",gridTemplateColumns:"32px minmax(0,1fr) auto",gap:10,alignItems:"start",padding:"12px 2px",borderBottom:`1px solid ${T.border}`}}>
                      <span style={{width:30,height:30,borderRadius:10,display:"grid",placeItems:"center",background:incoming?T.okBg:T.errBg,color:incoming?T.ok:T.err,fontWeight:900}}>{incoming?"+":"-"}</span>
                      <div style={{minWidth:0}}><div style={{fontSize:12,fontWeight:900,color:T.text}}>{historyLabel(entry)}</div><div style={{fontSize:10,color:T.muted,marginTop:3,whiteSpace:"normal",overflowWrap:"anywhere"}}>{entry.tgl||"Tanggal tidak tersedia"}{entry.ket?` · ${entry.ket}`:""}{wallet?` · ${wallet.nama}`:""}</div><div style={{fontSize:10,color:T.muted,marginTop:3}}>Saldo setelah: <strong style={{color:T.text}}>{IDRs(entry.balanceAfter)}</strong></div></div>
                      <div style={{fontSize:12,fontWeight:900,color:incoming?T.ok:T.err,whiteSpace:"nowrap"}}>{incoming?"+":"-"}{IDRs(Math.abs(entry.delta))}</div>
                    </div>;
                  }):<LaunchEmpty icon="GOAL" title="Belum ada perubahan saldo" desc="Setoran, penggunaan, peminjaman, dan pengembalian dana Goal akan muncul di sini." style={{padding:"30px 12px"}}/>}
                </div>
                <Btn onClick={()=>closeModal()} ch="Tutup" c={T.muted} outline style={{width:"100%",marginTop:14,padding:"10px"}}/>
              </>;
            })()}
      ...
      <div className="app-main-scroll" style={{flex:1,height:"var(--app-height, 100dvh)",overflowY:"auto",overflowX:"hidden",minWidth:0,maxWidth:"100%",width:0,background:T.bg,transition:"background .3s",WebkitOverflowScrolling:"touch",overscrollBehavior:"contain"}}>
        ...
        <div key={page} className={`page-in ${pageBack?"back":""}`} style={{display:page==="admin"?"none":undefined,padding:isMobile?`14px max(14px,env(safe-area-inset-right)) calc(80px + max(env(safe-area-inset-bottom),0px)) max(14px,env(safe-area-inset-left))`:"22px 28px 40px",maxWidth:1340,margin:"0 auto"}}>
          ...
          {page==="goals"&&<>
            ...
            <div className="feature-card-grid">
              {s.goals.map(g=><GoalCard key={g.id} g={g} dompetList={s.dompet} lang={lang} onDelete={()=>confirmDelete({title:"Hapus goal?",msg:`Goal "${g.nama}" dan riwayat tabungannya akan dihapus dari daftar.`,toastMsg:"Goal dihapus",onConfirm:()=>setS(p=>({...p,goals:p.goals.filter(x=>x.id!==g.id)}))})} onTambah={tambahGoalDana} onGunakan={gunakanGoalDana} onHistory={id=>setModal({type:"goalHistory",goalId:id})} onSourceChange={(id,dompetId)=>setS(p=>({...p,goals:p.goals.map(x=>x.id!==id?x:{...x,dompetId})}))} onSelesai={id=>setS(p=>({...p,goals:p.goals.map(x=>x.id!==id?x:{...x,selesai:true})}))}/>)}
          ...
          <div style={{padding:isMobile?`12px max(12px,env(safe-area-inset-right)) calc(80px + max(env(safe-area-inset-bottom),0px)) max(12px,env(safe-area-inset-left))`:"14px 14px 40px",maxWidth:1340,margin:"0 auto"}}>
            <Card ch={<div>
              <div style={{display:"flex",gap:12,alignItems:"flex-start",minWidth:0}}>
                ...
                <div style={{minWidth:0}}>
                  <div style={{fontSize:10,fontWeight:900,color:T.accent,letterSpacing:1.2,textTransform:"uppercase",marginBottom:4}}>Dashboard Admin</div>
                  <div style={{fontSize:18,fontWeight:900,color:T.text,letterSpacing:-.2,marginBottom:5}}>Kelola akses user dengan mudah</div>
                  <div style={{fontSize:12,color:T.muted,lineHeight:1.65}}>Cek pembayaran di Scalev, lalu pilih <b>Aktifkan</b> atau <b>Tolak</b>. Hanya akun yang menunggu yang perlu kamu tangani.</div>
                </div>
              </div>
            </div>} style={{marginBottom:18,padding:isMobile?14:"16px 18px"}}/>
            <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(4,1fr)",gap:12,marginBottom:18}}>
              {[
                {label:"Total",value:adminStats.total,color:T.accent,bg:T.accentBg},
                {label:"Menunggu",value:adminStats.pending_review,color:T.warn,bg:T.warnBg},
                {label:"Aktif",value:adminStats.approved,color:T.ok,bg:T.okBg},
                {label:"Ditolak",value:adminStats.rejected,color:T.err,bg:T.errBg},
              ].map((item)=>(
            ...
            <Card ch={<>
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>
                {[
                  ...
                  <button key={item.id} onClick={()=>setAdminFilter(item.id)} style={{padding:"8px 13px",borderRadius:999,border:`1px solid ${adminFilter===item.id?T.accent:T.border}`,background:adminFilter===item.id?T.accentBg:T.cardAlt,color:adminFilter===item.id?T.accent:T.text,fontSize:12,fontWeight:800,cursor:"pointer",fontFamily:"inherit"}}>
                    {item.label}
                  </button>
                ))}
              </div>
              <input value={adminQuery} onChange={e=>setAdminQuery(e.target.value)} placeholder="Cari nama atau email user" style={{...IS,marginBottom:14}}/>
              <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr auto",gap:12,marginBottom:14,padding:"12px 14px",borderRadius:12,background:T.cardAlt,border:`1px solid ${T.border}`}}>
                <div style={{fontSize:12,color:T.text,fontWeight:700}}>
                  {adminFilteredUsers.length} user ditemukan
                </div>
                <div style={{fontSize:11,color:T.muted}}>
                  Akun menunggu selalu tampil lebih dulu.
                </div>
              </div>
              <div style={{display:"grid",gap:12}}>
                ...
                {adminPagedUsers.map((user)=>(
                  <div key={user.uid} style={{background:T.cardAlt,border:`1px solid ${T.border}`,borderRadius:14,padding:14}}>
                    <div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"flex-start",flexWrap:"wrap",marginBottom:10}}>
                      ...
                      <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                        <span style={{fontSize:11,fontWeight:800,padding:"6px 10px",borderRadius:99,background:user.approvalStatus==="approved"?T.okBg:user.approvalStatus==="rejected"?T.errBg:T.warnBg,color:user.approvalStatus==="approved"?T.ok:user.approvalStatus==="rejected"?T.err:T.warn}}>
                          {user.approvalStatus==="approved"?"Aktif":user.approvalStatus==="rejected"?"Ditolak":"Menunggu"}
                        </span>
                    ...
                    <details style={{marginTop:12,paddingTop:10,borderTop:`1px solid ${T.border}`}}>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(2,minmax(0,1fr))",gap:"5px 16px",fontSize:11,color:T.sub,lineHeight:1.6,marginTop:9,overflowWrap:"anywhere"}}>
                        <span>Daftar: {user.createdAt ? new Date(user.createdAt).toLocaleString("id-ID") : "-"}</span>
                        <span>Login terakhir: {user.lastLoginAt ? new Date(user.lastLoginAt).toLocaleString("id-ID") : "-"}</span>
                        <span>Metode login: {user.authProvider || "-"}</span>
                        <span>UID: {user.uid}</span>
                        {user.buyerEmail&&<span>Email pembeli: {user.buyerEmail}</span>}
                ...
                {!adminFilteredUsers.length&&<LaunchEmpty icon="🛡️" title="Tidak ada user yang cocok" desc="Belum ada akun pada status ini atau pencarian tidak cocok." actionLabel="Tampilkan semua" onAction={()=>{setAdminFilter("all");setAdminQuery("");}} secondaryLabel="Refresh data" onSecondary={loadAdminUsers} style={{padding:"34px 18px"}}/>}
              </div>
